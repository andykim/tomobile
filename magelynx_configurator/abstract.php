...
abstract class Configurator_Abstract{
	...
	$svg = str_replace('fill-opacity:1','',$svg);
        ...
        foreach($layers_data as $layer_code => $layer_data){
            ...
            try{                
                if('svg' == $this->getFileExtension($layer_data['base_image'])){
                    ...
                    $this->_beforeOverlaySvgProceed($svg, $layer_code, $layer_data);
                    //handle overlay                                 
        ...
        $command = '#!/bin/bash
            ...
            offset=("'.join('" "',$exec['offset']).'")
                for index in {0..'.(count($exec['images'])-1).'}
                    ...
                    if [[ "${images[index]}" == *".svg" ]];
                    then
                        '.$this->getConvertPath().' -background none MSVG:${images[index]} -resize ${sizes[index]} -gravity center -background none -extent ${sizes[index]} -repage ${sizes[index]}${offset[index]}\! miff:-
                    else
                    ...
                    '.$this->getConvertPath().' -background none MIFF:- -layers merge +repage '.$result_path;                        
